...
function FloatingSprite({ url, link, home, description }) {
  ...
  useFrame(() => {
    ...
    const planeZ   = 0;
    const pWorld   = new THREE.Vector3(pointer.x, pointer.y, 0).unproject(camera);
    ...
    sprite.position.x = THREE.MathUtils.clamp(sprite.position.x, -xLimit, xLimit);
    sprite.position.y = THREE.MathUtils.clamp(sprite.position.y, -yLimit, yLimit);
  ...
  return (
    <sprite
        ...
        onPointerEnter={() => {ref.current.material.opacity = 0.8;}}
        onPointerLeave={() => {ref.current.material.opacity = 0.6;}}
        > 
